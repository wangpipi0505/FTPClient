...
int flag;		//文件上传下载标志，0为空闲，1为上传，2为下载
...
BOOL CFTPClientDlg::OnInitDialog()
{
	...
	Thread_Download_File = CreateThread(NULL,0,Thread_DownloadFile,this,0,NULL);//线程下载

	//HANDLE Thread_Download_File; 
	//Thread_Download_File = CreateThread(NULL,0,Thread_DownloadFile,this,0,NULL );//线程下载


	SetTimer(10,1000,NULL);		//test ，用于autodownload and autoupload
...
DWORD WINAPI CFTPClientDlg::Thread_UploadFile(LPVOID pParam)//接收udp数据
{
	while(1)
	{
		...
		*/
		...
		if(flag != 2)
		{
			if(fileNum != 0)	//当前没有下载文件
			{
				flag = 1;		//下载列表有任务，进入下载状态
				...
				for(int i = 0;i<fileNum;i++)
				{
					...
					if (pCurl->upload(uploadfile_info.upfile_fullpath,uploadfile_info.upfile_name))
					{
						...
						vUpload.vec_up.erase(vUpload.vec_up.begin());		//文件上传失败时同样删除文件，避免进入死循环
					}
					else 
					{
		...
		if(flag != 1)
		{
			if(file_down_num != 0)
			{
				...
				for(int i = 0;i<file_down_num;i++)
				{
					...
					vDownload.vec_down.erase(vDownload.vec_down.begin());		//文件下载失败时同样删除文件，避免进入死循环
				}
			}
			else
			{
				if(flag == 2)
					flag = 0;//下载任务结束，将标志位设为空闲
				Sleep(3000);
			}
		}
	}
...
void CFTPClientDlg::On32784()			//核对下这块代码，是否肯定能执行到
{
	// TODO: 在此添加命令处理程序代码
...
void CFTPClientDlg::AutoDownload()//定时下载
{
	...
	/*CFileFind file;  */
	//CFtpFileFind ftpfind(pFtpConnection);//定义FTP查找
	CFtpFileFind ftpfind(pThreadFtpConnection);//定义FTP查找pThreadFtpConnection
	BOOL bContinue = ftpfind.FindFile(str);  
	while(bContinue)  
	{  
		bContinue = ftpfind.FindNextFile();  
		...
		if (strfilename.GetLength()!=0)
		{
			...
			WideCharToMultiByte(CP_OEMCP,NULL,strDownloadFile,-1,source_file_path_after,dwNum_source_file_path,0,NULL);//源路径
			...
			//char *pathpath = strcat(server_path_after,server_file_name_after);
			strcpy(downloadfile_info.downfile_fullpath,source_file_path_after);
			strcpy(downloadfile_info.downfile_name,dest_file_path_after);
...
void CFTPClientDlg::AutoUpload()//定时上传
{
	...
	{  
		...
		{  
			...
			if(strstr(temp_filename,".path.txt"))//读取txt文件中的内容		//文件名后缀为.path.txt时读取解析上传文件信息
			{
				FILE *temp_pathFile;
				...
				if (getfile_size == nullptr)
				{
					...
					CLOG("打开文件%s失败,err= %d",temp_path,err);
					//return 0;
				}
				else
				{
					...
					if(file_size != 0)
					{
						...
						if(temp !=NULL)
						{
							...
							if(!DeleteFile(tmp_path))		//删除任务记录txt文件
							{
								int err = GetLastError();
						...
						delete sizefile;
						vUpload.vec_up.push_back(uploadfile_info);		//存入txt文件信息

						strcpy(uploadfile_info.upfile_fullpath,temp_path);
						strcpy(uploadfile_info.upfile_name,fileName_needUp);
						vUpload.vec_up.push_back(uploadfile_info);		//存入上传文件到列表
					}
				}
...
//使用方法1.先定义char *p； 2.调用本函数 3.使用char*p 4.delete p释放内存
