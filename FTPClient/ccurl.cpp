...
long long  CCurl::get_file_size(FILE *file)
{
	if (file == nullptr)
	{
		CLOG("文件句柄无效，获取大小失败");
...
void CCurl::setUploadOpt(FileInfo *fileinfo, const char* url)
{
	...
	if (nSize == 0)
	{
		CLOG("File's size is zero,file name is %s ",fileinfo->filename.c_str());
		return;
	}
...
void  CCurl::getRemoteFileSize(FileInfo *fileinfo, const char* url)
{
	if (curl == nullptr || url == nullptr || m_strUserPwd.empty())
	{
		CLOG("null or empty error");
	...
	fileinfo->filesize = get_file_size(fileinfo->file);
	if (fileinfo->filesize == 0)
	{
		CLOG("local File's size is zero,filename is %s",fileinfo->filename.c_str());
		//return;
	}
	...
	//设置http 头部处理函数
	curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, getcontentlengthfunc);
	curl_easy_setopt(curl, CURLOPT_HEADERDATA, &(fileinfo->remotefilesize));
...
bool CCurl::upload(char *localFullName, char *remoteFileName/* = nullptr*/)
{
	...
	// open file
	FileInfo srcfile,tmpfile;
	srcfile.filename = localFullName;
	srcfile.file = fopen(localFullName, "rb+");	
	if (nullptr == srcfile.file)
	{
		int err=GetLastError();
		CLOG("File open error :%s, err = %d",localFullName,err);
		return false;
	}

	tmpfile.filename = strRemoteFile + ".temp";
	getRemoteFileSize(&srcfile, strRemoteFile.c_str());
	getRemoteFileSize(&tmpfile, tmpfile.filename.c_str());

	if(tmpfile.remotefilesize > srcfile.remotefilesize)
		srcfile.remotefilesize = tmpfile.remotefilesize;


	// curl_easy_setopt for uploading
	setUploadOpt(&srcfile, strRemoteFile.c_str());
	if(srcfile.remotefilesize >0)
	{
		if (srcfile.file == nullptr)
		{
			CLOG("文件句柄无效，获取大小失败,name is %s",srcfile.filename);
			return 0;
		}
		else
			_fseeki64(srcfile.file,srcfile.remotefilesize,SEEK_SET);
		if(srcfile.remotefilesize == srcfile.filesize)
		{
			//if(feof(srcfile.file))
			CLOG("服务器文件%s已存在，不传输",srcfile.filename.c_str());
			return true;
		}
		//续传
		curl_easy_setopt(curl, CURLOPT_APPEND, 1L);
	...
	if (res != CURLE_OK)
	{
		CLOG("curl_easy_perform() failed: %s, file is %s", curl_easy_strerror(res),srcfile.filename.c_str());
		return false;
	}
	
	CLOG("文件上传成功: %s", srcfile.filename.c_str());
	return true;
}
